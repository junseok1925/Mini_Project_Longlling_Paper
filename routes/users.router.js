...
//댓글 신고 기능 try
router.put(
    '/posts/router.get("/posts/:postId/comments/:commentId',
    authMiddleware,
    async (req, res) => {
        try {
            ...
            const commentUserId = await Comments.findOne({
                attributes: ['UserId'],
                where: { commentId },
            });
            console.log(commentUserId); //오류 발생 시 여기서 출력이 {userid} , userid 로 되는지, 제대로 나오는지 확인 필요
            ...
            if (!commentUserId) {
                return res.status(404).json({
                    errorMessage: '댓글 쓴 사용자가 존재하지 않습니다.',
                });
            }
            //banCount +1
            await Users.increment('banCount', {
                where: { userId: commentUserId },
            });

            //bancount=3이면 사용자정보 삭제

            const banCount = await Users.findOne({ where: { banCount } });
            if ((banCount = 3)) {
                await destroy({
                    where: { userId: commentUserId },
                });
                ...
                return res
                    .status(201)
                    .json({ message: '사용자를 신고하였습니다' });
            }
        } catch (err) {
            console.log(err);
            return res
                .status(400)
                .json({ errorMessage: '신고에 실패하였습니다.' });
        }
    }
);
